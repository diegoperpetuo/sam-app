...
const App = () => {
  ...
  const est_processo0 = async () => {
    try {
      const response = await fetch(
        `rw-plc-master-production.up.railway.app/plc/siemens/read?ip=${config_pedido.ip}&data_type=${config_pedido.data_type}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const fetchIdPeca3 = async () => {
    try {
      const response = await fetch(
        `http://192.168.3.240
        // :8000/plc/siemens/read?ip=${config_pedido.ip}&data_type=${config_pedido.data_type}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
